<template>
  <div class="w100 tab-content-item pressure" v-if="!active">
    ...
    <div class="trend">
      ...
      <div class="w100 lineChart" id="lineChart"></div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getStatistic() {
      ...
      getBSUnitPointNum({ unitId: this.unitId, systemId: "7963" }).then(res => {
  ...
  mounted() {
    this.baseInfo = [
      { label: "单位名称", value: this.unitData.unitName },
      { label: "地址", value: this.unitData.address },
      { label: "报警数量", value: this.unitData.alarmNum },
      { label: "未处置数量", value: this.unitData.undisposeNum },
      { label: "经度", value: this.unitData.lon },
      { label: "纬度", value: this.unitData.lat },
      { label: "风险", value: this.unitData.riskName },
      { label: "单位电话", value: this.unitData.phone }
    ];
