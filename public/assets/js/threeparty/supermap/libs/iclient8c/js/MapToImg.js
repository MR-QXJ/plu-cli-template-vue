(function() {
  ...
  var LAYER_COUNT = 0;
  var LAYER_LENGTH = 0;
  // update dingxing 2019.12.23 支持地图容器下显示截图预览
  var parentDom = null;
  window.MapToImg = PrintMap;

  PrintMap.excute = function(map, el) {
    if (el) {
      parentDom = typeof el === "string" ? document.getElementById(el) : el;
    }
    ...
    for (var i = 0; i < layers.length; ) {
      if (layers[i].CLASS_NAME == "SuperMap.Layer.Markers") {
        ...
        layers[i].CLASS_NAME == "SuperMap.Layer.GOAnimationLayer" ||
        layers[i].CLASS_NAME == "SuperMap.Layer.PlottingLayer.Temporary" ||
        (layers[i].CLASS_NAME == "SuperMap.Layer.PlottingLayer" &&
  ...
  // update dingxing 2019.12.23, draw函数没传map对象导致报错
  function draw(map, img, i, imgUrls) {
    ...
    if (LAYER_COUNT >= LAYER_LENGTH) {
      ...
      // update dingxing 2019.12.23 添加class以便于自定义样式
      panel.setAttribute("class", "supermap-screenshot-preview");
      ...
      // update dingxing 2019.12.23 添加class以便于自定义样式
      buttonPanel.setAttribute("class", "preview-tool");
  ...
  function getVectorLayerData(layer, map, callback) {
    ...
    window.setTimeout(
      (function(printLayer, map, callback) {
        return function() {
  ...
  function getPlottingLayerData(layer, map, callback) {
    var printLayer,
      features1 = [],
      features = layer.features,
      printLayer = new SuperMap.Layer.PlottingLayer(
        "PRINT_LAYER",
        layer.serverUrl,
        { visibility: true, renderers: ["Canvas"] }
      );
